...
import '../../constants/enum/locale_enum.dart';
...
class NetworkManager {
    ...
    dio = Dio(BaseOptions(
      ...
      headers: {
        if (LocaleManager.instance
            .containsKey(Locale.TOKEN.value)!)
          'Authorization':
  ...
  Future post<T extends BaseModel>(String path, dynamic data, T model,
      ...
      bool isBaseResponse = true, isFile = false}) async {
    try {
      var body = data is Map || data is FormData ? data : data.toJson();
      var response;

      if(isFile){
        response = await dioFile.post(path, data: body, queryParameters: queryParameters);
      }else {
        response = await dio.post(path, data: body, queryParameters: queryParameters);
      }
  ...
  Future put<T extends BaseModel>(String path, dynamic data, T model,
    ...
    try {
      ...
      if(isFile){
        ...
        response = await dio.put(path, data: body, queryParameters: queryParameters);
      }
      if (response.statusCode == HttpStatus.ok) {
        if (isBaseResponse) {
          return _baserResponseConverter(response.data, model: model);
